...
#include "terminal_wrapper.h"
  ...
  asm 
  {
    ...
    /* LEDs als digitale Ausgabe konfigurieren ========================== */
     ...
     clr.b    MNP_GPIO_CLRTC        //LEDS aus, siehe Figure 15-3


     clr.l    d1                    // Zähler, wird dann auf die LEDs ausgegeben
     ...
     loop:
        ...
        clr.l    d0
        wait1:                              // Warteschleife gegen Schalterprellen
           addq.l   #1, d0                  // (engl:: "switch bouncing")
           tst.w    d0
           bne      wait1
        ...
        clr.l    d0                         
        move.b   MNP_GPIO_PINDATA_SETNQ, d0  // Taster via PINDATA_SETNQ abfragen
        andi.l   #MNP_GPIO_PORTNQ_NQ5, d0    // Check NQ5 auf 1 (SW1 nicht gedrückt)
        ...
        clr.l    d0
        wait2:                              // Warteschleife gegen Schalterprellen
           addq.l   #1, d0                  // (engl:: "switch bouncing")
           tst.w    d0
           bne      wait2
        ...
        addq.l   #1,d1                      // Zähler erhöhen und
        move.b   d1, MNP_GPIO_PORTTC        // Ausgabe auf LEDs
